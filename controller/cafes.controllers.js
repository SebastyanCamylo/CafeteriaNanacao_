...
const addCafes = async (req, res) => {
    ...
    try {
        ...
        if(existeUncafeConEseId) res.status(400).send({ message:'There is already a café with that id'})
        else {
    cafes.push(cafe)
...
const updateCafes = async(req, res) => {
    ...
    .status(400)
    .send ({
        message: 'The id of the parameter does not match the id of the received coffee',
    });
    ...
    if (cafeIndexFound >= 0 ) {
        ...
        res
        .status(400)
        .send({ message: 'No coffee found with that id'});
    }
    } catch (error) {
        console.log(error)
        handleErrorResponse(res,error.code)
